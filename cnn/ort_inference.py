...
sess = ort.InferenceSession('./mixed.onnx', providers=['CPUExecutionProvider'], sess_options=options, device_id=0, device_type=device)
...
input = np.random.random((1, 72, 32, 1)).astype(np.float32)
...
# print(ps.get_stats_profile().func_profiles['run'].cumtime)
# ps.print_stats()
