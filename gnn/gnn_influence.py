...
def graph_plot(data, idx, x_in):
    ...
    inf = influence(data[idx], x_in, abs=True).detach().cpu().numpy()
    plt.figure(figsize=(5, 11.5))
    plt.xlim(0, 31.0)
    plt.ylim(0, 71.0)
    plt.scatter(x[:,0], x[:,1], c=inf)
    plt.scatter(x[x_in,0], x[x_in,1], c='r', marker="s", s=55)
    #max_inf = max(inf[np.arange(len(inf)) != x_in])
    #sm = plt.cm.ScalarMappable(cmap='rainbow', norm=plt.Normalize(vmin=0, vmax=max_inf))
    plt.colorbar()
